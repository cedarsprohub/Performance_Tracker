...
export default function UpskillingTab({ idToken, data, scores, notifyAndReload, setError }) {
  ...
  const byKey = useMemo(() => {
    const map = {};
    learning.forEach((l) => { map[`${l.staffName}|${l.courseName}`] = l.status; });
    return map;
  }, [learning]);
  ...
  async function cycleCell(staffName, courseName) {
    const current = byKey[`${staffName}|${courseName}`] || null;
            ...
            <tbody>
              {staff.map((s) => (
                <tr key={s}>
                  ...
                  {courses.map((c) => {
                    const status = byKey[`${s}|${c}`];
                    const meta = status ? LEARN_META[status] : null;
                    return (
                      <td key={c} className="center">
                        <button
                          ...
                          title={meta ? meta.label : 'Not started'}
                          onClick={() => cycleCell(s, c)}
                        />
                      </td>
                    );
                  })}
