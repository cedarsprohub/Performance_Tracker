          ...
          {byStaff.map(({ s, items }) => (
            <div key={s} className="task-row">
              ...
              <div className="task-chips">
                ...
                {items.map((t) => {
                  const isLive = String(t.id).startsWith('live-');
                  return (
                    <span key={t.id} className="task-chip" style={{ background: TASK_META[t.status].color + '22' }}>
                      {isLive ? (
                        <span className="chip-live" title="Synced from the WorkPlan sheet — edit it there">
                          <span className="dot" style={{ background: TASK_META[t.status].color }} />
                          {t.label} <span className="live-tag">synced</span>
                        </span>
                      ) : (
                        <>
