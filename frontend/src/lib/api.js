...
export async function apiRequest(token, action, payload = {}) {
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      throw new Error(`"${action}" took too long to respond (25s) and was cancelled. The Sheet it reads from may be large — try again, or ask Claude about trimming what gets scanned.`);
    }
    throw new Error(`Could not reach the backend for "${action}": ${err.message}`);
  } finally {
...
export function scanSinceFor(periodStart) {
  const d = new Date();
  d.setMonth(d.getMonth() - 2);
  const pad = (n) => String(n).padStart(2, '0');
  const twoMonthsAgo = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;
  return periodStart < twoMonthsAgo ? periodStart : twoMonthsAgo;
}
