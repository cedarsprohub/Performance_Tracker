...
function handleRequest_(body) {
  ...
  if (action === 'liveTasks') return jsonOut_({ tasks: readLiveWorkPlan_(readStaff_(sheets.staffSheet), cleanText_(body.since)), user: email });
...
// ---------- Team KPI (reads a different spreadsheet live, with a short cache) ----------
function readTeamKpi_() {
  const cache = CacheService.getScriptCache();
  const cached = cache.get('kpi_v1');
  if (cached) return JSON.parse(cached);

  const ss = SpreadsheetApp.openById(TEAMS_TRACKER_ID);
  const result = KPI_SHEETS.map((entry) => {
  ...
  cache.put('kpi_v1', JSON.stringify(result), 120); // 2 minutes — sheet data doesn't change second-to-second
  return result;
}
...
function extractKpiSheet_(sheet) {
  ...
  // One bulk read for the whole used range instead of per-cell calls — this
  // is what was causing "failed to fetch" timeouts on larger KPI sheets.
  const values = sheet.getRange(1, 1, maxRow, maxCol).getValues();

  const monthRow = values[1]; // row 2
  const monthCols = []; // { col (0-indexed), monthKey: 'YYYY-MM' }
  for (let c = 1; c < maxCol; c += 1) {
    const label = cleanText_(monthRow[c]);
    if (!label) continue;
    const key = normalizeMonth_(label);
    if (key) monthCols.push({ col: c, monthKey: key });
  }
  if (monthCols.length === 0) return [];

  const kpis = [];
  for (let r = 3; r < maxRow; r += 1) {
    const kpiName = cleanText_(values[r][0]);
    if (!kpiName) continue;
    const months = {};
    monthCols.forEach(({ col, monthKey }) => {
      const target = parseKpiNumber_(values[r][col]);
      const achieved = parseKpiNumber_(values[r][col + 1]);
...
function readLiveWorkPlan_(staffList, sinceDate) {
  const cacheKey = 'workplan_v2_' + (sinceDate || 'default');
  const cache = CacheService.getScriptCache();
  ...
  // Default window: at least the last 2 months, further back if the caller
  // (the app's own period selector) asks for an earlier date.
  const twoMonthsAgo = new Date();
  twoMonthsAgo.setMonth(twoMonthsAgo.getMonth() - 2);
  const defaultSince = Utilities.formatDate(twoMonthsAgo, Session.getScriptTimeZone(), 'yyyy-MM-dd');
  const effectiveSince = (sinceDate && sinceDate < defaultSince) ? sinceDate : defaultSince;

  // Phase 1: cheap read of just the date header row to find which columns
  // are actually within the window, so the big read below can be narrowed.
  const headerRow = sheet.getRange(2, 2, 1, maxCol - 1).getValues()[0];
  let minCol = null; // 1-indexed sheet column
  let maxColInWindow = null;
  headerRow.forEach((v, i) => {
    if (!(v instanceof Date)) return;
    const dateStr = Utilities.formatDate(v, Session.getScriptTimeZone(), 'yyyy-MM-dd');
    if (dateStr < effectiveSince) return;
    const col = i + 2; // +2 because headerRow started at column 2
    if (minCol === null) minCol = col;
    maxColInWindow = col;
  });
  if (minCol === null) return []; // nothing in range

  const windowWidth = maxColInWindow - minCol + 1;

  // Phase 2: bulk read staff names (column 1, all rows) and the narrowed
  // date-column window only — not the whole sheet's history.
  const namesCol = sheet.getRange(1, 1, maxRow, 1).getValues();
  const dateHeaderWindow = sheet.getRange(2, minCol, 1, windowWidth).getValues()[0];
  const values = sheet.getRange(1, minCol, maxRow, windowWidth).getValues();
  const backgrounds = sheet.getRange(1, minCol, maxRow, windowWidth).getBackgrounds();

  const dateCols = []; // { col (0-indexed into `values`), date }
  dateHeaderWindow.forEach((v, i) => {
    if (v instanceof Date) dateCols.push({ col: i, date: Utilities.formatDate(v, Session.getScriptTimeZone(), 'yyyy-MM-dd') });
  });

  const nameRows = [];
  for (let r = 2; r < maxRow; r += 1) {
    const v = namesCol[r][0];
  ...
  cache.put(cacheKey, JSON.stringify(tasks), 120); // 2 minutes
  return tasks;
}

function readLiveUpskilling_(staffList) {
  const cache = CacheService.getScriptCache();
  const cached = cache.get('upskilling_v1');
  if (cached) return JSON.parse(cached);
  ...
  const result = { courses, learning };
  cache.put('upskilling_v1', JSON.stringify(result), 120); // 2 minutes
  return result;
}
